  ...
  <div class="fade-up">
    ...
    <div class="card" style="padding:0">
      <div class="table-wrapper">
        <table>
          <thead>
            <tr><th>Solicitante</th><th>Área</th><th>Tipo</th><th>Actividad</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Motivo</th><th>Estado</th><th>Aprobado Por</th><th>Acciones</th></tr>
          </thead>
          <tbody>
            <tr v-if="cargando"><td colspan="10" style="text-align:center;padding:2rem;color:var(--text-muted)">Cargando...</td></tr>
            <tr v-else-if="!permisos.length"><td colspan="10"><div class="empty-state"><div class="icon">📋</div><p>Sin permisos</p></div></td></tr>
      ...
      <div class="modal">
        ...
        <div class="grid-2">
          <div class="form-group" style="grid-column:1/-1">
            ...
            <input v-model="form.solicitante" type="text" class="form-control" placeholder="Nombre del encargado o estudiante"/>
          </div>
          <div class="form-group">
            <label class="form-label">Área</label>
          ...
          <div class="form-group">
            ...
            <select v-model="form.tipo" class="form-control">
              <option>Permiso de Presentación</option>
              <option>Permiso de Práctica</option>
              <option>Permiso de Evento</option>
              <option>Ausencia Justificada</option>
              <option>Otro</option>
            </select>
          </div>
          <div class="form-group" style="grid-column:1/-1">
          ...
          <div class="form-group" style="grid-column:1/-1">
            ...
            <input v-model="form.motivo" type="text" class="form-control" placeholder="Razón del permiso..."/>
          </div>
          <div class="form-group">
            <label class="form-label">Estado</label>
...
import { exportPermisosExcel, exportToWord } from '../utils/export.js'
...
function doExportExcel() { exportPermisosExcel(permisos.value,{}); showExport.value=false }
function doExportWord() {
  exportToWord('Registro de Permisos',
    ['Solicitante','Área','Tipo','Actividad','Fecha Inicio','Fecha Fin','Motivo','Estado','Aprobado Por'],
    permisos.value.map(p=>[p.solicitante,p.area,p.tipo,p.actividad||'',p.fechaInicio,p.fechaFin,p.motivo,p.estado,p.aprobadoPor||'']),
    `Permisos_${new Date().toISOString().split('T')[0]}`
  )
